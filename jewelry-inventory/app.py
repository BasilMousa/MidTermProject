...
def add_item():
    if request.method == 'POST':
        ...
        functions.add_jewelry_item(inventory, item_type, category, cost_price)
...
def mark_sold():
    if request.method == 'POST':
        ...
        try:
            ...
            if selling_price <= 0:
                raise ValueError
            _, updated_item = functions.mark_item_sold(inventory, item_id, selling_price)
            if not updated_item:
...
def update_item():
    if request.method == 'POST':
        try:
            ...
            updated = functions.update_item(inventory, item_id, updated_data)
            if not updated:
...
@app.route('/profit')
def profit_summary():
    summary = functions.calculate_profit_summary(inventory)
    return render_template('profit.html', summary=summary)

@app.route('/summary')
def inventory_summary():
    summary = functions.get_inventory_summary(inventory)
