...
def get_inventory_summary(inventory):
    ...
    total_selling = sum(item['selling_price'] or 0 for item in inventory if item['status'] == 'sold')
    total_profit = total_selling - sum(item['cost_price'] for item in inventory if item['status'] == 'sold')
